...
public:
    bool handlePacket(IOComponent * pConn, Packet *packet)
	{
		...
		Normal2PubPacketProcessor::getSingleton().addPacket((ClientPacket *)packet);
		return true;
    }
};


PubGameServer::PubGameServer()
{
}
...
// 	if (!COne2OnePVPMgr::getSingleton().loadCfg()){
// 		Athena::logger->trace("加载1v1配置出错!");
// 		return false;
// 	}
...
// 	PubGame1SecTimerCB * pubcb1Sec = ATHENA_NEW PubGame1SecTimerCB();
// 	m_timerMgr.addTimer(pubcb1Sec);
...
// 	return true;
// }
bool PubGameServer::startListen()
{
	char listenParam[250] = {0};
	snprintf(listenParam,sizeof(listenParam),"tcp:%s:%d",m_svrInfo.m_ip,m_svrInfo.m_port);
    IOComponent *ioc = m_netConnMgr.listen(listenParam,g_spliter,g_serverAdapter);
...
void PubGameServer::createTimer()
{
	GameTimerCB  * cb = ATHENA_NEW GameTimerCB(0);
	m_timerMgr.addTimer(cb);
	GameTimer1SecCB * cb1Sec = ATHENA_NEW GameTimer1SecCB();
...
bool PubGameServer::loadAllClient()
{
	return true;
}
...
{
	Athena::global["ServerName"] = "PubGameServer";
	//解析命令行
	g_parseCmdLine(argc, argv);
	//通用的初始化
	g_initCommon(argc,argv);
	
    PubGameServer gameServer;
	//设置信号量
