...
public:
    ...
    void synGame();
    //检查数据是否合法
    bool check(UINT32 groupId, std::vector<stLimitPet> & petVec);
    //插入记录
    void insert(UINT32 groupId, std::vector<stLimitPet> & petVec);
    //修改记录
    void update(UINT32 groupId, std::vector<stLimitPet> & petVec);
    //是否是更新记录
    bool isUpdate(UINT32 groupId);
    //删除记录
    void del(UINT32 groupId);
private:
    std::set<UINT32> m_groupSet;
