...
{
	//std::string in = "https://itunes.apple.com/cn/app/luo-li-you-sha-qi-ci-shi-dai/id1072064226?l=zh&ls=1&mt=8";
	//std::string out = HttpUtility::URLEncode(in);
	//printf("%s\n",out.c_str());
	//printf("decode:%s\n",HttpUtility::URLDecode(out).c_str());
	...
	if (fp != NULL)	{
		while (!feof(fp)){
			...
			std::string out = HttpUtility::URLEncode(szLine);
			printf("%s\n",out.c_str());
			printf("decode:%s\n",HttpUtility::URLDecode(out).c_str());
		}
		fclose(fp);
