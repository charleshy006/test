...
#ifdef _HDZ_DEBUG
	Athena::logger->trace("加载了荣耀商店的配置,size = %d", m_allActivitys.size());
...
//获得活动信息
CGloryfashionCfg::stActivityInfo * CGloryfashionCfg::getActicityInfo(UINT32 cur)
{
	std::map<UINT32,stActivityInfo>::iterator it = m_allActivitys.begin();
