...
static bool parseItem(TiXmlElement *pDrawItem, std::vector<CDevilCallCfg::stDrawItem>& itemVec, UINT32 & totalRate) {
    if (!pDrawItem) {
...
static bool parseCertain(TiXmlElement *pCertain, CDevilCallCfg::stCertainInfo & certainInfo) {
    if (!pCertain) {
...
void CDevilCallCfg::getAllDevilCall(std::map<UINT32, UINT32> & devilsMap) {
...
UINT32 CDevilCallCfg::getDevilReward(UINT32 bossId, UINT8 rewardType) {
...
UINT32 CDevilCallCfg::getOpenCondition(UINT32 bossId) {
    std::map<UINT32, stDevilInfo>::iterator it =
        m_devilMap.find(bossId);
    if (it != m_devilMap.end()) {
...
UINT32 CDevilCallCfg::getDrawId(UINT32 bossId) {
    std::map<UINT32, stDevilInfo>::iterator it =
        m_devilMap.find(bossId);
    if (it != m_devilMap.end()) {
...
void CDevilCallCfg::getDrawInfo(UINT32 drawId, UINT32 & moneyType, UINT32 & needMoney) {
...
void CDevilCallCfg::drawItem(UINT32 drawId, std::map<UINT32, UINT32> & drawMap) {
    std::map<UINT32, stDrawInfo>::iterator it = m_drawMap.find(drawId);
    if (it == m_drawMap.end()) {
        return ;
    }
...
void CDevilCallCfg::drawCertailItem(UINT32 drawId, UINT32 drawNum, std::map<UINT32, UINT32> & drawMap) {
